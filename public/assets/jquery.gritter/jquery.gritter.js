...
(function($){
	...
	*/
	$.gritter.options = {
		...
		tipo: '', // Tipos: 'notice', 'error', 'info', 'success'
	...
	*/
	var Gritter = {
		...
		_item_count: 0,
		_is_setup: 0,
		_tpl_close: '<div class="gritter-close"><i class="fa fa-times"></i></div>',
		...
		*/
		add: function(params){
			...
			var title = params.title, 
				...
				grid_id = "gritter-notice-wrapper"+position,
				tipo = params.tipo || $.gritter.options.tipo;
			...
			switch(tipo){
				case 'erro':
					...
					break;
				case 'alerta':
					item_class += " bg-yellow";
					break;
				case 'sucesso':
					item_class += " bg-green";
					break;
				default:
					item_class += " bg-light-blue";
					break;
			}

			this._tpl_wrap = '<div id="'+grid_id+'" class="gritter-notice-wrapper"></div>';

			console.log(grid_id);

			this._verifyWrapper(grid_id);

			console.log(this._tpl_wrap);
			...
			$('#'+grid_id).addClass(position).append(tmp);
		...
		*/
		_countRemoveWrapper: function(unique_id, e, manual_close, grid_id){
			...
			if($('#'+grid_id+' .gritter-item-wrapper').length == 0){
				$('#'+grid_id).remove();
			}
		...
		* @private
		*/  
		_verifyWrapper: function(grid_id){
