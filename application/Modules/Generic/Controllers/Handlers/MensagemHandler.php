...
class MensagemHandler extends Controller
{
    public function GetTable($pagina = 1, $saida = 0, $lixeira = 0,$termo=""){
        ...
        $msgs = $this->unitofwork->Get(new Mensagem(), "Assunto LIKE '%".$termo."%' OR Conteudo LIKE '%".$termo."%'")->Join($this->unitofwork->Get(new MensagemPessoa(), "mp.PessoaId = '".$UsuarioId."' AND mp.Apagada = '".$lixeira."'"), "m.MensagemId", "mp.MensagemId")->Select("mp", new MensagemPessoa())->Skip($totalPagina)->Take(50)->ToList();
            ...
            $msgs->For_Each(function ($msg, $i) use($saida, $lixeira) {
                ...
                <tr class="trMsgitem">
                    ...
                    <td class="mailbox-subject"><a
                            href="<?= \Libs\Helper::getUrl("ler", "mensagem", $msg->MensagemPessoaId) ?>"><?= $msg->Mensagem->Assunto; ?></a>
                    </td>
                    <td class="mailbox-name"><?= Helper::Abreviar($msg->Mensagem->Pessoa->Nome); ?></td>
